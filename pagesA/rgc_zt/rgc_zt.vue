<template>
	<view class="wrap_box">
		...
		<view class="main_box">
			...
			<image v-else class="main_bg" :src="$service.getimg('/static_wx/images/rgczt_bg.png')" mode="widthFix"></image>
			<view class="cz_box">
				...
				<picker @change="bindPickerChange_c" :value="car_type" :range="cc_array" range-key="title">
					<view class="xq_li">
						...
						<!-- <input type="text" class="xq_r" v-model="engine" placeholder="请输入发动机号后4位"> -->
	...
	export default {
		...
		methods: {
			...
			go_fuc(){
				...
				uni.navigateTo({
					url:'/pagesA/rgc_zt_order/rgc_zt_order?type=1&cs='+cs
				})
			},
			focus ( type ) {
				// let typeArr = {
