		...
		methods: {
			...
			getadd(){
				...
				uni.chooseLocation({
					success: res => {
						...
						console.log('当前位置的：');
						console.log(res);
						that.master_addcontent=res.address//地址
						that.master_lng=res.longitude//经度  
						that.master_lat=res.latitude//纬度  
						that.master_address=res.name
					},fail(err) {
						console.log(err);
					}
				});
			...
			mk_fuc(){
				...
				var datas={
					...
					sign_time:that.datas_cs.sign_time,
					nj_time:that.datas_cs.nj_time,
					car_type:that.datas_cs.car_type,
					nj_type:that.datas_cs.nj_type,
					is_accident:that.datas_cs.is_accident,
					
					master_name:that.master_name,
					master_phone:that.master_phone,
					master_address:that.master_address,
					master_lat:that.master_lat,
					master_lng:that.master_lng,
					master_addcontent:that.master_addcontent,
					txm:that.txm,
			...
			gook_fuc(code){
				...
				setTimeout(function(){
					...
					// })
					uni.redirectTo({
						url:'/pagesA/rgc_nj_order1/rgc_nj_order1?type='+that.options.type+'&code='+code
					})
				},1000)
			},
